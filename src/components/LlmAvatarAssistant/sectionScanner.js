...
/**
 * Pull the real, visible content of a section so the LLM can be grounded in
 * it: the section's own text, plus alt text from any images (icon-only
 * content like a tech-stack row has no text nodes at all).
 */
function extractSectionText(el, maxLength = 500) {
  if (!el) return '';
  const text = (el.textContent || '').replace(/\s+/g, ' ').trim();
  const altTexts = Array.from(el.querySelectorAll('img[alt]'))
    .map((img) => img.getAttribute('alt'))
    .filter(Boolean);
  const combined = altTexts.length ? `${text} (${altTexts.join(', ')})` : text;
